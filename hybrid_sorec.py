	...
	for step in xrange(steps):
		...
		for i,j,val in zip(R.row, R.col, R.data):
			if pre_i == -1:
				pre_i = i
			if pre_i != i:
				pre_u = Csc.getcol(pre_i)
				u_tr = pre_u.T.dot(TRU)
				# print TRU[pre_i,:], u_tr
				U[pre_i,:] = U[pre_i,:] - alpha * (sig + lam_uv * U[pre_i,:] + lam_t * TRU[pre_i,:] - lam_t * u_tr)
				for x in range(pre_i+1,i):
					pre_u = Csc.getcol(x)
					u_tr = pre_u.T.dot(TRU)
...
nU, nV, em = matrix_factorize(R, U, V, C_norm, C, Z, K)
# nR = np.dot(nU,nV.T)
# nnR = bou(nR)
# aR = get(nnR)
# print aR
